#!/bin/sh
# install.sh — install the tabstride CLI on macOS and Linux from GitHub Releases.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Tencent/TabStride/main/install.sh | sh
#
# Environment overrides:
#   TABSTRIDE_REPO         GitHub owner/repo (default: Tencent/TabStride)
#   TABSTRIDE_VERSION      Pin CLI version (default: latest from version.json)
#   TABSTRIDE_INSTALL_DIR  Install directory (default: $HOME/.local/bin)
#   TABSTRIDE_BRANCH       Branch for install_sh raw URL metadata only (unused here)

set -eu

REPO="${TABSTRIDE_REPO:-Tencent/TabStride}"
INSTALL_DIR="${TABSTRIDE_INSTALL_DIR:-$HOME/.local/bin}"
...
main() {
  ...
  if [ -n "${TABSTRIDE_VERSION:-}" ]; then
    version="${TABSTRIDE_VERSION#v}"
  ...
  tag="cli-v${version}"
  archive="tabstride-v${version}-${triple}.tar.gz"
  ...
  mkdir -p "$INSTALL_DIR"
  install -m 0755 "${tmp_dir}/tabstride" "${INSTALL_DIR}/tabstride"

  log "installed tabstride to ${INSTALL_DIR}/tabstride"
  ensure_path

  if command -v tabstride >/dev/null 2>&1; then
  "${INSTALL_DIR}/tabstride" --version
  else
    log "verify install: ${INSTALL_DIR}/tabstride --version"
  fi
