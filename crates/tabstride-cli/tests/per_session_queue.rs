...
use tabstride_protocol::tools::{SessionStartParams, SessionStartResult};
...
async fn spawn_daemon() -> (daemon::DaemonHandle, PathBuf) {
    ...
    drop(probe);
    let sock = tempfile_path("tabstride-test-queue");
...
async fn same_session_dispatches_run_after_previous_completes() {
    ...
    for i in 0..total {
        ...
        let sid = tabstride::daemon::sessions::SessionId(session_id.clone());
        let i = i as u32;
...
async fn session_stop_fast_fails_while_tool_is_in_flight() {
    ...
    let stop_result = tabstride::daemon::sessions::stop_session(
        &state.browsers,
        &state.sessions,
        &state.tool_queues,
    ...
    assert!(
        matches!(
            stop_result,
            Err(tabstride::daemon::sessions::StopSessionError::SessionBusy)
        ),
    ...
    tabstride::daemon::sessions::stop_session(
        &state.browsers,
        &state.sessions,
        &state.tool_queues,
        &state.session_interrupts,
...
async fn concurrent_same_session_dispatch_returns_session_busy_immediately() {
    ...
    assert_eq!(
        rpc.data
            ...
            .and_then(|v| v.as_str()),
        Some(tabstride::rpc_reason::SESSION_BUSY)
    );
...
async fn dispatches_for_different_sessions_do_not_block_each_other() {
    ...
    let sid_a = tabstride::daemon::sessions::SessionId(session_a.clone());
    ...
    let sid_b = tabstride::daemon::sessions::SessionId(session_b.clone());
...
async fn dispatch_returns_session_not_found_for_unknown_session() {
    ...
    let result = queues
        .dispatch(
            &tabstride::daemon::sessions::SessionId("zzzz".into()),
            Method::ToolTabList,
