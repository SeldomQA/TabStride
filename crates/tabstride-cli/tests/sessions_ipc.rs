...
async fn spawn_daemon() -> (daemon::DaemonHandle, PathBuf) {
    spawn_daemon_with_connect_wait(tabstride::daemon::browsers::EXTENSION_CONNECT_WAIT).await
}
...
async fn spawn_daemon_with_connect_wait(connect_wait: Duration) -> (daemon::DaemonHandle, PathBuf) {
    ...
    let sock = tempfile_path("tabstride-test-ipc");
...
async fn session_start_errors_without_browser() {
    ...
    let mut ipc = IpcClient::connect(&sock).await.unwrap();
    let result: Result<serde_json::Value, tabstride_protocol::RpcError> = ipc
        .call::<(), _>("s-1", Method::SessionStart, None, Duration::from_secs(2))
        .await
        .unwrap();
    let err = result.expect_err("expected no_browser_connected error");
    assert_eq!(err.code, tabstride_protocol::ErrorCode::NoBrowserConnected);
    handle.shutdown().await;
}
...
async fn session_start_errors_with_multiple_browsers() {
    ...
    let entries: Vec<tabstride_protocol::system::BrowserStatusEntry> = browsers
        .iter()
...
async fn session_start_with_unknown_label_returns_not_found() {
    ...
    assert_eq!(err.code, tabstride_protocol::ErrorCode::NotFound);
    handle.shutdown().await;
}
...
async fn session_start_ambiguous_label_returns_invalid_params() {
    ...
    assert_eq!(err.code, tabstride_protocol::ErrorCode::InvalidParams);
...
async fn session_window_closed_event_purges_session() {
    ...
    let session = tabstride::daemon::sessions::Session {
    ...
    let event = tabstride_protocol::EventFrame {
        event: tabstride_protocol::EventKind::SessionWindowClosed,
    ...
    ws.send(Message::Text(
        serde_json::to_string(&tabstride_protocol::Frame::Event(event)).unwrap(),
    ))
    .await
    .unwrap();
...
async fn session_stop_self_heals_when_extension_reports_not_found() {
    ...
    state
        .tool_queues
        .spawn(tabstride::daemon::sessions::SessionId("yyyy".into()));
    assert_eq!(state.sessions.len(), 1);
    ...
    let responder = tokio::spawn(async move {
        loop {
                ...
                && req.method == Method::ToolSessionStop
            {
                let reply = ResponseFrame {
                    id: req.id,
                    body: ResponseBody::Err(tabstride_protocol::RpcError {
                        code: tabstride_protocol::ErrorCode::NotFound,
...
async fn reserve_id_loops_until_vacant_and_caps_attempts() {
    ...
    let registry = std::sync::Arc::new(tabstride::daemon::sessions::SessionRegistry::new());
    let browser = tabstride::daemon::browsers::BrowserId("collision-browser".into());
