//! Spawn `tabstride daemon start` (default detached mode), wait for daemon.json,
//! then `tabstride daemon stop` and verify the daemon is gone.
...
fn run(home: &std::path::Path, args: &[&str]) -> std::process::Output {
    Command::new(tabstride_bin())
        .env("TABSTRIDE_HOME", home)
        .env("RUST_LOG", "warn")
        .args(args)
        .output()
        .expect("spawn tabstride command")
}
