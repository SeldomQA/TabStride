...
//! the `TABSTRIDE_HOME` env var.

use std::sync::Mutex;

use tabstride::daemon::info::{self, DaemonInfo};
use tabstride::daemon::paths::{self, TABSTRIDE_HOME_ENV};
...
fn read_returns_none_before_any_write() {
    with_temp_home(|| {
        let _home = paths::ensure_tabstride_home().unwrap();
...
fn write_then_read_round_trip() {
    with_temp_home(|| {
        let home = paths::ensure_tabstride_home().unwrap();
