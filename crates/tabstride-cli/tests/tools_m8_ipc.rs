...
//! fake extension and assert the wire shapes line up with tabstride-protocol.
...
async fn spawn_daemon() -> (daemon::DaemonHandle, PathBuf) {
    ...
    drop(probe);
    let sock = tempfile_path("tabstride-test-tools-m8");
...
async fn session_stop_return_failures_keep_session_registered() {
    ...
    assert!(
        handle
            .state()
            .sessions
            .get(&tabstride::daemon::sessions::SessionId(session_id))
            .is_some(),
