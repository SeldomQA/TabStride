...
fn with_doctor_env<R>(
    ...
    let saved = [
        "TABSTRIDE_HOME",
        "TABSTRIDE_BROWSER_WAIT_MS",
        "TABSTRIDE_DOCTOR_BROWSER_WAIT_MS",
    ]
    .into_iter()
    ...
    unsafe {
        std::env::set_var("TABSTRIDE_HOME", tabstride_home);
        std::env::remove_var("TABSTRIDE_BROWSER_WAIT_MS");
        std::env::remove_var("TABSTRIDE_DOCTOR_BROWSER_WAIT_MS");
...
async fn spawn_daemon() -> (daemon::DaemonHandle, PathBuf) {
    ...
    let sock = tempfile_path("tabstride-test-browser-wait");
...
async fn system_status_waits_for_late_extension_handshake() {
    ...
    let status_task = tokio::spawn(async move {
        ...
        ipc.call::<StatusParams, tabstride_protocol::StatusResult>(
            "status-wait",
            Method::SystemStatus,
            Some(StatusParams {
...
async fn browser_list_waits_for_late_extension_handshake() {
    ...
    struct ListReply {
        browsers: Vec<tabstride_protocol::system::BrowserStatusEntry>,
    }
...
async fn system_status_without_wait_returns_immediately_when_empty() {
    ...
    let status = ipc
        .call::<StatusParams, tabstride_protocol::StatusResult>(
            "status-now",
            Method::SystemStatus,
...
async fn doctor_reports_extension_connected_after_handshake() {
    ...
    let tabstride_home = home.path().join("tabstride");
    std::fs::create_dir_all(&tabstride_home).unwrap();
    ...
    std::fs::write(
        tabstride_home.join("daemon.json"),
    ...
    let tabstride_home_for_doctor = tabstride_home.clone();
    let checks = tokio::task::spawn_blocking(move || {
        with_doctor_env(
            &tabstride_home_for_doctor,
            &[("TABSTRIDE_BROWSER_WAIT_MS", "0")],
...
async fn doctor_waits_for_late_extension_on_verified_daemon() {
    ...
    let doctor_task = tokio::task::spawn_blocking(move || {
        with_doctor_env(
            &tabstride_home_for_doctor,
            &[("TABSTRIDE_DOCTOR_BROWSER_WAIT_MS", "500")],
            || doctor::run(Output::Json),
        )
    });
