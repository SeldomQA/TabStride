...
async fn spawn_daemon() -> (daemon::DaemonHandle, PathBuf) {
    ...
    let sock = tempfile_path("tabstride-test-handshake");
...
async fn handshake_skew_when_protocol_minor_differs() {
    ...
    let client = state
        .browsers
        .get(&tabstride::daemon::browsers::BrowserId(TEST_EXT_ID.into()))
...
async fn status_surfaces_version_skew_for_skewed_browser() {
    ...
    let state = handle.state();
    let (tx, _rx) = mpsc::unbounded_channel::<tabstride_protocol::Frame>();
    let client = std::sync::Arc::new(tabstride::daemon::browsers::BrowserClient {
        id: tabstride::daemon::browsers::BrowserId("skew-only-test".into()),
        browser_name: "chrome".into(),
        browser_version: "131.0".into(),
        extension_version: "9.9.9".into(),
        extension_protocol_version: "1.1".into(),
        label: "Older".into(),
        sink: tabstride::daemon::browsers::BrowserSink { tx },
        pending: Mutex::new(tabstride::daemon::browsers::Pending::default()),
        generation: tabstride::daemon::browsers::next_browser_generation(),
        connected_at_ms: 0,
        version_skew: true,
    });
