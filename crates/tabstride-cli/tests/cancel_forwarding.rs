...
use tabstride_protocol::tools::SessionStartResult;
...
async fn spawn_daemon() -> (daemon::DaemonHandle, PathBuf) {
    ...
    let sock = tempfile_path("tabstride-test-cancel");
...
impl PromoteDelayHook {
    fn enable(delay: Duration) -> Self {
        ...
        tabstride::daemon::inflight::__set_promote_delay_for_tests(delay);
        Self { _lock: lock }
    }
}
...
impl Drop for PromoteDelayHook {
    fn drop(&mut self) {
        tabstride::daemon::inflight::__set_promote_delay_for_tests(Duration::from_millis(0));
    }
}
