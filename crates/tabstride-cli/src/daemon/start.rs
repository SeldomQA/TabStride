//! Daemon entrypoints (`tabstride daemon start|stop|restart`).
//!
...
//!
//! Detachment uses a hidden `TABSTRIDE_DAEMONIZED=1` env handoff: when the
...
pub(crate) const DAEMONIZED_ENV: &str = "TABSTRIDE_DAEMONIZED";
...
/// `tabstride daemon stop` entrypoint.
...
pub fn run_foreground(cfg: DaemonConfig) -> Result<()> {
    ...
    runtime.block_on(async move {
        ...
        tokio::select! {
            _ = wait_for_shutdown() => {
                info!("tabstride daemon shutting down (signal)");
            }
            res = idle_task => {
                if matches!(res, Ok(Some(()))) {
                    info!("tabstride daemon shutting down (idle)");
                }
            }
        }
...
/// `~/.tabstride/` and (best-effort) also mirror to stderr. Returns the
...
fn validate_existing_start(args: &StartArgs, status: &StatusResult) -> Result<()> {
    ...
    {
        return Err(anyhow::anyhow!(
            "daemon already running on ws port {}; use `tabstride daemon restart --port {port}` to change it",
            status.ws_port
        ));
    }
    if args.session_idle.is_some() || args.daemon_idle.is_some() {
        return Err(anyhow::anyhow!(
            "daemon already running; use `tabstride daemon restart` to apply idle timeout changes"
        ));
    }
    Ok(())
}
