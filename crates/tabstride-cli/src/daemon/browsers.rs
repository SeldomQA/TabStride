...
//! entry so `tabstride status` shows a single row per physical browser.
...
pub struct Pending {
    waiters: HashMap<RpcId, oneshot::Sender<tabstride_protocol::ResponseFrame>>,
}

impl Pending {
    pub fn register(&mut self, id: RpcId) -> oneshot::Receiver<tabstride_protocol::ResponseFrame> {
        let (tx, rx) = oneshot::channel();
        self.waiters.insert(id, tx);
        rx
    }

    pub fn resolve(&mut self, frame: tabstride_protocol::ResponseFrame) -> bool {
...
pub struct BrowserClient {
    ...
    /// from `chrome.storage.local.tabstride_instance_id`, NOT the Chromium
    ...
    /// so `tabstride browsers` and the M10.1 `multiple_browsers_online`
