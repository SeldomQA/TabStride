...
//! IPC server (CLI peer). Minimal scope: enough for `tabstride session start`,
//! `tabstride session stop`, `tabstride session list` and the matching
...
/// shows up first (matches the `tabstride browsers` output order).
...
pub enum StartSessionError {
    ...
    MultipleBrowsersOnline {
        ...
        /// hint instead of asking the user to run `tabstride browsers` again
        /// (M10.1).
    ...
    pub fn code(&self) -> &'static str {
        match self {
            ...
            StartSessionError::ExtensionError(err) => match err.code {
                tabstride_protocol::ErrorCode::Timeout => "timeout",
                tabstride_protocol::ErrorCode::Cancelled => "cancelled",
                _ => "protocol_error",
            },
        }
    }
...
pub async fn stop_session(
    ...
    match queues
        .dispatch_after_closing(
            session_id,
            tabstride_protocol::Method::ToolSessionStop,
            params,
            timeout_dur,
        )
        .await
    {
        ...
        Err(DispatchError::Rpc(err)) => {
            ...
            // leaving an orphan row visible to `tabstride session list`
            // (review M4/M5 round 3 I-R3-2).
            if matches!(err.code, tabstride_protocol::ErrorCode::NotFound) {
