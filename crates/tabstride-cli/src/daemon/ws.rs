    ...
    let callback = move |req: &Request, response: Response| {
        ...
        *captured_origin_for_cb.lock().unwrap() = Some(origin);
        ...
        resp.headers_mut().append(
            "x-tabstride-server",
            HeaderValue::from_static("tabstride-daemon"),
        );
        Ok(resp)
    };
...
async fn drive_connection(
    ...
    if !matches!(request.method, tabstride_protocol::Method::SystemHandshake) {
    ...
    let version_skew = match compat {
        HandshakeCompat::Reject { reason } => {
            let resp = ResponseFrame {
                id: request.id.clone(),
                body: ResponseBody::Err(RpcError {
                    code: tabstride_protocol::ErrorCode::VersionTooOld,
...
async fn handle_inbound_text(state: &Arc<DaemonState>, client: &Arc<BrowserClient>, text: &str) {
    ...
    match frame {
        ...
        Frame::Event(ev) => match ev.event {
            tabstride_protocol::EventKind::SessionWindowClosed => {
                handle_session_window_closed(state, &client.id, &ev.payload);
            }
            tabstride_protocol::EventKind::SessionUserInterrupt => {
