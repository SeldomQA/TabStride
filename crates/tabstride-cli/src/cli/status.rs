//! `tabstride status` — auto-spawn the daemon, query `system.status`, render.
...
/// Run `tabstride status`. Returns the result so callers (tests, doctor) can
/// reuse it.
...
fn render_human(s: &StatusResult) {
    if !s.version_skew_browsers.is_empty() {
        ...
        // a user sees on `tabstride status`. Yellow text via the bare ANSI
