//! `tabstride update` — check for and install CLI updates.
...
pub const DEFAULT_MANIFEST_URL: &str =
    "https://github.com/Tencent/TabStride/releases/latest/download/version.json";
...
impl UpdateManifest {
    ...
    pub fn update_candidate(
        ...
        let current = Version::parse(current_version.trim_start_matches('v'))
            .context("parse current tabstride version")?;
        ...
        let asset = self
            .assets
            .get(platform_key)
            .with_context(|| format!("no tabstride release asset for platform `{platform_key}`"))?
            .clone();
...
impl ArchiveKind {
    pub fn from_url(url: &str) -> Result<Self> {
        if url.ends_with(".tar.gz") || url.ends_with(".tgz") {
            ...
            bail!("unsupported tabstride archive type: {url}");
        }
    }
}
    ...
    let Some(candidate) = manifest.update_candidate(current_version, platform)? else {
        return render_report(
            ...
            &UpdateReport {
                ...
                asset_url: None,
                install_action: None,
                message: format!("tabstride {current_version} is already up to date"),
            },
        );
    };
    ...
    if args.check {
        return render_report(
            ...
            &UpdateReport {
                ...
                install_action: None,
                message: format!(
                    "tabstride {} is available (current {})",
...
pub fn current_platform_key() -> Result<&'static str> {
    match (std::env::consts::OS, std::env::consts::ARCH) {
        ...
        (os, arch) => bail!("unsupported platform for tabstride auto-update: {os}-{arch}"),
    }
}
...
pub fn verify_sha256(bytes: &[u8], expected_hex: &str) -> Result<()> {
    ...
    if actual != expected {
        bail!("downloaded tabstride archive checksum mismatch: expected {expected}, got {actual}");
    }
    Ok(())
}
...
pub fn update_hint_for_manifest(
    ...
    Ok(manifest
        ...
        .map(|candidate| {
            format!(
                "A new tabstride version is available: {} -> {}. Run `tabstride update`.",
                candidate.current, candidate.latest
            )
        }))
}
...
fn update_hint_for_cache(cache: &UpdateCheckCache, current_version: &str) -> Option<String> {
    ...
    (latest_version > current).then(|| {
        format!("A new tabstride version is available: {current} -> {latest_version}. Run `tabstride update`.")
    })
}
...
pub fn maybe_spawn_background_check(flags: &super::GlobalFlags, command: &super::Command) {
    ...
    let _ = std::thread::Builder::new()
        .name("tabstride-update-check".to_string())
    ...
    match format {
        Format::Human => {
            ...
            if matches!(report.install_action, Some("staged")) {
                println!(
                    "restart your terminal and run `tabstride --version` to verify the staged update"
                );
            }
        }
        Format::Json => {
...
pub fn extract_tabstride_binary(archive_bytes: &[u8], kind: ArchiveKind) -> Result<Vec<u8>> {
    match kind {
        ArchiveKind::TarGz => extract_tabstride_from_tar_gz(archive_bytes),
        ArchiveKind::Zip => extract_tabstride_from_zip(archive_bytes),
    }
}
...
fn extract_tabstride_from_tar_gz(archive_bytes: &[u8]) -> Result<Vec<u8>> {
    let decoder = GzDecoder::new(Cursor::new(archive_bytes));
    let mut archive = tar::Archive::new(decoder);
    for entry in archive.entries().context("read tabstride tar.gz entries")? {
        let mut entry = entry.context("read tabstride tar.gz entry")?;
        let path = entry.path().context("read tabstride tar.gz entry path")?;
        if path.file_name().is_some_and(|name| name == "tabstride") {
            let mut bytes = Vec::new();
            entry
                .read_to_end(&mut bytes)
                .context("read tabstride binary from tar.gz")?;
            return Ok(bytes);
        }
    }
    bail!("tabstride binary not found in tar.gz archive");
}

fn extract_tabstride_from_zip(archive_bytes: &[u8]) -> Result<Vec<u8>> {
    let cursor = Cursor::new(archive_bytes);
    let mut archive = zip::ZipArchive::new(cursor).context("read tabstride zip archive")?;
    for index in 0..archive.len() {
        ...
        let name = std::path::Path::new(file.name())
            ...
            .and_then(|name| name.to_str())
            .unwrap_or("");
        if name == "tabstride.exe" || name == "tabstride" {
            let mut bytes = Vec::new();
            file.read_to_end(&mut bytes)
                .context("read tabstride binary from zip")?;
            return Ok(bytes);
        }
    }
    bail!("tabstride binary not found in zip archive");
}
...
mod tests {
    ...
    fn parses_new_and_legacy_manifest_asset_shapes() {
        let manifest = UpdateManifest::from_slice(
            br#"{
                ...
                "version": "0.2.0",
                "tag": "cli-v0.2.0",
                "release_url": "https://github.com/Tencent/TabStride/releases/tag/cli-v0.2.0",
                "assets": {
                    "darwin-arm64": {
                        "url": "https://example.test/tabstride.tar.gz",
                        "sha256": "abc123"
                    },
        ...
        assert_eq!(
            ...
            "https://example.test/tabstride.tar.gz"
        );
        assert_eq!(
    ...
    #[test]
    fn extracts_tabstride_binary_from_tar_gz_archive() {
        let archive = tar_gz_with_tabstride(b"new binary");
        let extracted = extract_tabstride_binary(&archive, ArchiveKind::TarGz).unwrap();
        assert_eq!(extracted, b"new binary");
    }

    #[test]
    fn extracts_tabstride_binary_from_zip_archive() {
        let archive = zip_with_tabstride_exe(b"windows binary");
        let extracted = extract_tabstride_binary(&archive, ArchiveKind::Zip).unwrap();
    ...
    fn replaces_binary_at_path() {
        ...
        let target = tmp.path().join("tabstride");
    ...
    fn staged_replacement_paths_are_next_to_target() {
        let target = std::path::Path::new("/tmp/tabstride.exe");
        let paths = staged_replacement_paths(target, 42).unwrap();
        assert_eq!(
            paths.binary_path,
            std::path::Path::new("/tmp/tabstride.exe.update-42")
        );
        assert_eq!(
            paths.script_path,
            std::path::Path::new("/tmp/tabstride.exe.update-42.cmd")
        );
    }
    ...
    fn update_hint_is_generated_only_for_newer_versions() {
        ...
        assert_eq!(
            hint.as_deref(),
            Some("A new tabstride version is available: 0.1.7 -> 0.2.0. Run `tabstride update`.")
        );
        assert!(
    ...
    fn tar_gz_with_tabstride(binary: &[u8]) -> Vec<u8> {
    ...
    fn zip_with_tabstride_exe(binary: &[u8]) -> Vec<u8> {
        let mut cursor = Cursor::new(Vec::new());
        {
            let mut writer = zip::ZipWriter::new(&mut cursor);
            writer
                .start_file("tabstride.exe", zip::write::SimpleFileOptions::default())
                .unwrap();
