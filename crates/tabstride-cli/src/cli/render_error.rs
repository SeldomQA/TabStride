...
//!   * `tabstride-cli` subcommands that surface their own bespoke
...
pub fn info_for(code: ErrorCode) -> RenderInfo {
    match code {
        ErrorCode::UnknownMethod => RenderInfo {
            ...
            hint: Some(
                "upgrade both tabstride CLI and the tabstride extension; CLI and daemon may be on mismatched protocol versions",
            ),
        ...
        ErrorCode::Unsupported => RenderInfo {
            ...
            hint: Some(
                "check `tabstride --version` and the tabstride changelog to confirm whether the feature is available",
            ),
            exit_code: 1,
        },
        ErrorCode::InvalidParams => RenderInfo {
            ...
            hint: Some(
                "check your command arguments; run `tabstride <cmd> --help` to see the expected format",
            ),
            exit_code: 1,
        },
        ...
        ErrorCode::PermissionDenied => RenderInfo {
            ...
            hint: Some(
                "tabs outside an Agent Window must first be borrowed via `tabstride tab borrow <tab-id> --session <id>`",
            ),
            exit_code: 1,
        },
        ErrorCode::Timeout => RenderInfo {
            ...
            hint: Some(
                "retry the command; if timeouts persist, check `tabstride logs` and confirm the browser is still responding",
            ),
            exit_code: 4,
        },
        ...
        ErrorCode::ProtocolError => RenderInfo {
            summary: "protocol error communicating with tabstride daemon",
            hint: Some(
                "ensure CLI and daemon protocol versions match — check `protocol version` in `tabstride status`",
            ),
            exit_code: 2,
        },
        ...
        ErrorCode::VersionTooOld => RenderInfo {
            ...
            hint: Some(
                "upgrade both tabstride CLI and the tabstride extension so both sides satisfy `min_compatible_protocol`",
            ),
            exit_code: 5,
        },
        ...
        ErrorCode::NoBrowserConnected => RenderInfo {
            ...
            hint: Some(
                "open the tabstride extension in your browser and wait for the popup to show \"connected\"",
            ),
            exit_code: 1,
        },
    }
...
pub fn info_for_error(code: ErrorCode, data: Option<&serde_json::Value>) -> RenderInfo {
    ...
    match (code, reason) {
        ...
        (ErrorCode::PermissionDenied, reason::BORROW_CONFLICT) => RenderInfo {
            ...
            hint: Some(
                "return the tab from the borrowing session via `tabstride tab return <tab-id> --session <id>` or stop that session",
            ),
            exit_code: base.exit_code,
        },
        ...
        (ErrorCode::InvalidParams, reason::TARGET_NOT_SELECT) => RenderInfo {
            ...
            hint: Some(
                "choose a native <select> ref or selector from the latest snapshot, or use `tabstride click` / `tabstride fill` for other controls",
            ),
            exit_code: base.exit_code,
        },
        (ErrorCode::InvalidParams, reason::OPTION_NOT_FOUND) => RenderInfo {
            ...
            hint: Some(
                "rerun `tabstride snapshot` or `tabstride get-html` to list valid option values, then pass them via `--value`",
            ),
            exit_code: base.exit_code,
        },
        ...
        (ErrorCode::InvalidParams, reason::TAB_NOT_ACTIVE) => RenderInfo {
            ...
            hint: Some(
                "select the tab with `tabstride tab select <tab-id> --session <id>` or omit `--tab-id` to capture the Agent Window's active tab",
            ),
            exit_code: base.exit_code,
        },
...
mod tests {
    ...
    /// here (rather than in tabstride-protocol) so a test failure shows up
