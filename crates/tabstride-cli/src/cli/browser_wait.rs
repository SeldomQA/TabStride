...
/// Browser wait for `tabstride status` / `tabstride browsers`. Override in tests via
/// `TABSTRIDE_BROWSER_WAIT_MS`.
pub fn browser_connect_wait() -> Duration {
    wait_from_env("TABSTRIDE_BROWSER_WAIT_MS").unwrap_or(DEFAULT_BROWSER_CONNECT_WAIT)
}

/// Browser wait for `tabstride doctor`. Falls back to [`browser_connect_wait`] when
/// `TABSTRIDE_DOCTOR_BROWSER_WAIT_MS` is unset so tests can shorten only doctor.
pub fn doctor_browser_connect_wait() -> Duration {
    wait_from_env("TABSTRIDE_DOCTOR_BROWSER_WAIT_MS").unwrap_or_else(browser_connect_wait)
}
