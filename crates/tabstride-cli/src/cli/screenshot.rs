//! `tabstride screenshot` — capture a PNG of the Agent Window's active tab
...
pub struct ScreenshotArgs {
    ...
    /// Optional `@eN` ref from the last `tabstride snapshot`. Crops the
    /// capture to the matching element.
    #[arg(long = "ref")]
    pub ref_: Option<String>,

    /// Output PNG path. Defaults to `$TMPDIR/tabstride-screenshot-<unix-ms>.png`.
    #[arg(long)]
...
fn default_out_path() -> PathBuf {
    ...
    let ts = SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    dir.push(format!("tabstride-screenshot-{ts}.png"));
    dir
}
...
mod tests {
    ...
    fn default_out_path_lives_in_tmpdir() {
        ...
        assert!(
            p.file_name()
                .and_then(|n| n.to_str())
                .map(|s| s.starts_with("tabstride-screenshot-"))
                .unwrap_or(false)
        );
    }
}
