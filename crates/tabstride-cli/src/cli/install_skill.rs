//! `tabstride install-skill` — install tabstride SKILL.md into agent harnesses.
...
pub struct InstallSkillArgs {
    ...
    /// Overwrite an existing `tabstride` skill installation.
    #[arg(long)]
    pub force: bool,
}
...
fn render_list(reports: &[crate::skill_install::HarnessReport], output: Output) -> Result<()> {
    match output {
        Output::Human => {
            print_harness_table(
                reports,
                "Agent harnesses supported by `tabstride install-skill`:\n",
            );
            eprintln!(
