//! `tabstride doctor` — guided diagnostics + repair hints.
...
/// Chrome Web Store listing for the tabstride extension.
...
fn check_home_writable() -> CheckResult {
    let name = "tabstride home writable";
    match paths::ensure_tabstride_home() {
        Ok(home) => CheckResult::ok(name, home.display().to_string()),
        Err(err) => CheckResult::fail(
            name,
            format!("{err:#}"),
            "ensure $HOME or $TABSTRIDE_HOME is writable",
        ),
    }
}
...
fn check_skill_up_to_date() -> CheckResult {
    ...
    if !report.errors.is_empty() {
        ...
        return CheckResult::fail(
            name,
            detail,
            "re-run `tabstride install-skill --force --harness <id>` for the failing harness",
        );
    }
...
fn check_daemon_running(state: &DaemonState) -> CheckResult {
    ...
    match state {
        ...
        DaemonState::ReadError(err) => CheckResult::fail(
            ...
            "check permissions on ~/.tabstride",
        ),
...
fn check_version_compatible(status: Option<&StatusResult>) -> CheckResult {
    ...
    if ok {
        ...
        CheckResult::fail(
            ...
            "upgrade or restart tabstride so CLI and daemon speak the same protocol version",
        )
    }
}

/// `tabstride doctor` check: every connected browser should use the same
...
fn check_browsers_protocol_compatible(status: Option<&StatusResult>) -> CheckResult {
    ...
    CheckResult::fail(
        ...
        "upgrade the tabstride extension or tabstride CLI so both sides use the same protocol version",
    )
}
...
mod m2_tests {
    use super::*;
    use tabstride_protocol::StatusResult;
    use tabstride_protocol::system::{BrowserStatusEntry, VersionSkewEntry};
    ...
    fn fake_status(browsers: Vec<BrowserStatusEntry>, skew: Vec<VersionSkewEntry>) -> StatusResult {
        StatusResult {
            ...
            uptime_secs: 0,
            ws_port: 0,
            sock_path: "/tmp/tabstride.sock".into(),
            browsers,
            sessions: Vec::new(),
