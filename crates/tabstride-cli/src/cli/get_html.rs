//! `tabstride get-html` — dump raw DOM HTML (M6.4). Optionally narrows to a
//! single subtree by `@eN` ref. Defaults its `--max-bytes` budget to
//! the tabstride-protocol spec (`524288`) when the user does not override
...
pub struct GetHtmlArgs {
    ...
    /// Optional `@eN` ref from the last `tabstride snapshot`. Scopes the
