//! `tabstride snapshot` — aria-tree snapshot of the Agent Window's active
//! tab (M6.3). Text format is human-readable with `@eN` refs the
//! agent feeds back into `tabstride click`, `tabstride fill`, etc.

use std::path::PathBuf;

use anyhow::Context;
use tabstride_protocol::Method;
use tabstride_protocol::tools::{SnapshotParams, SnapshotResult};
use clap::Args;
