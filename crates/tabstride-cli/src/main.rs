...
use tabstride::cli::error::{CliError, Format, render};
...
fn dispatch(cli: Cli, format: Format) -> Result<(), CliError> {
    ...
    match cli.command {
        ...
        Command::Assert(args) => cli::assertions::dispatch(args, format),
...
fn init_cli_tracing(flags: &tabstride::cli::GlobalFlags) {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new(format!("tabstride={level}")));
