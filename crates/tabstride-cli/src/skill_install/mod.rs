//! Install the bundled tabstride `SKILL.md` into agent harness skill directories.
...
pub const SKILL_DIR_NAME: &str = "tabstride";
...
/// JSON envelope for `tabstride install-skill --json` (single document, includes exit metadata).
#[derive(Debug, Clone, Serialize)]
...
pub fn run_interactive_prompt(reports: &[HarnessReport]) -> Result<Vec<HarnessId>> {
    ...
    if candidates.is_empty() {
        bail!(
            "no Agent harness detected on this machine; use `tabstride install-skill --list` to check paths, or use `--harness <id>` to install manually"
        );
    }
