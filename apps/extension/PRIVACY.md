# TabStride — Privacy Policy

**Last updated:** May 25, 2026

This Privacy Policy describes how the **TabStride** browser extension (the "Extension") handles information when you install and use it. TabStride is published as part of the open-source [TabStride](https://github.com/Tencent/TabStride) project. The source code is publicly auditable.

If you have questions about this policy, please open an issue in the project repository.

---

## 1. What TabStride Does

TabStride is a local automation bridge that lets AI coding agents (such as Cursor, Claude Code, Codex, and OpenClaw) drive a Chromium browser through a command-line tool (`tabstride`). The Extension communicates exclusively with a local daemon running on your own computer. **The Extension itself does not contact any remote server, cloud service, or AI provider.**

## 2. Single Purpose

The Extension's single purpose is to expose browser automation primitives (navigation, DOM observation, screenshots, clicks, form filling, tab management) to a locally running TabStride daemon over a WebSocket connection on `127.0.0.1`, so that an AI agent invoked by the user can interact with web pages on the user's behalf.
...
|---|---|---|
| **Web page content** | The DOM, accessibility tree, HTML, and visible-tab screenshots of pages opened in the TabStride-controlled "Agent Window," or in user tabs the user explicitly approves for borrowing. | Required so the AI agent can read pages, locate elements, and verify results. |
...
- **`debugger`** — Attach the Chrome DevTools Protocol to the Agent Window so the agent can observe and interact with pages. Used only on tabs explicitly under TabStride's control.
...
Because the Extension communicates only with `127.0.0.1`, no data is exposed to the network. Users should still avoid running TabStride in untrusted environments, since any local process able to bind to `127.0.0.1:52800` could send commands to the Extension. Run TabStride only on machines you control.

## 11. Open Source and Auditability

TabStride is open source. Anyone can verify the claims in this policy by reading the source code in the project repository. The Extension contains no obfuscated or minified code paths that hide network calls.
...
For privacy-related questions or requests, please open an issue in the TabStride project repository.
