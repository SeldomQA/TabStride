...
/**
 * Mirror of tabstride-protocol `TabInfo` (see
 * crates/tabstride-protocol/src/tools/tabs.rs).
 */
...
// --- M8 payload mirrors (tabstride-protocol/src/tools/tabs.rs) ---
  ...
  if (aborted(deps.signal, "tab_create")) {
    ...
    if (typeof tab.id === "number") {
      try {
        ...
        console.debug("[tabstride tab_create] cleanup after abort failed", cleanupErr);
      }
    }
...
async function moveTabForBorrow(
  ...
  if (aborted(signal, "tab_borrow")) {
    try {
      ...
      console.debug("[tabstride tab_borrow] rollback move failed", rollbackErr);
    }
    return { code: "cancelled", message: "tab_borrow aborted" };
  }
  return null;
...
async function executeBorrowCore(
  ...
  try {
    ...
    console.debug("[tabstride tab_borrow] activate after move failed", err);
  }
...
function resetAgentOverlaysInReturnedTab(
  ...
  void getAgentOverlayResetApi(deps)
    ...
    .catch((err) => {
      console.debug("[tabstride tab_return] agent overlay reset failed", err);
    });
}
...
export async function returnBorrowedTab(
  ...
  try {
    ...
    console.debug("[tabstride tab_return] original window gone, falling back", err);
    originalAlive = false;
  }
  if (!originalAlive) {
    fallback = true;
