...
export function attachBorrowNotificationClickHandler(deps: {
  ...
  const handler = (notificationId: string) => {
    ...
    void deps.windows.update(pending.windowId, { focused: true }).catch((err) => {
      console.debug("[tabstride borrow] focus on notification click failed", err);
    });
    void deps.notifications.clear(notificationId).catch((err) => {
      console.debug("[tabstride borrow] clear notification failed", err);
    });
  };
  ...
  clear: (id) =>
    new Promise<boolean>((resolve) => {
      try {
        ...
        console.debug("[tabstride borrow] notifications.clear threw", err);
        resolve(false);
      }
    }),
};
...
const DEFAULT_NOTIFICATION_COPY: BorrowNotificationCopy = {
  title: "TabStride: Agent wants to borrow a tab",
...
async function listConfirmationCandidates(
  ...
  try {
    ...
    console.debug("[tabstride borrow] windows.getLastFocused failed", err);
  }
  ...
  try {
    ...
    console.debug("[tabstride borrow] windows.getAll failed", err);
  }
...
export async function requestBorrowConfirmation(
  ...
  try {
    ...
    console.warn("[tabstride borrow] could not get tab info — using fallback title", {
      tabId,
  ...
  if (candidates.length === 0) {
    console.warn(
      "[tabstride borrow] no injectable user window available — proceeding without overlay",
      {
        tabId,
      },
    );
  ...
  return new Promise<boolean>((resolve) => {
    ...
    const cleanupNotification = () => {
      ...
      void notificationsApi.clear(notificationId).catch((err) => {
        console.debug("[tabstride borrow] notification clear failed", {
          notificationId,
          error: err instanceof Error ? err.message : String(err),
        });
      });
    ...
    const onAbort = () => {
      if (activeMessageTabId !== null) {
        ...
        void tabsApi.sendMessage(activeMessageTabId, cancelMessage).catch((err) => {
          console.debug("[tabstride borrow] cancel message failed", {
            tabId,
    ...
    timeout = setTimeout(() => {
      console.info("[tabstride borrow] confirmation timed out — proceeding", {
        tabId,
        messageTabId: activeMessageTabId,
      });
      settle(true);
    ...
    if (notificationsApi) {
      ...
      notificationsApi
        ...
        .catch((err) => {
          console.debug(
            "[tabstride borrow] notifications.create failed — continuing overlay-only",
            {
              tabId,
              error: err instanceof Error ? err.message : String(err),
            },
    ...
    const tryCandidate = (
      ...
      if (index >= candidates.length) {
        ...
        console.warn(
          "[tabstride borrow] every candidate user window failed sendMessage — awaiting notification button or timeout",
          {
            tabId,
            attempts: errorTrail,
          },
      ...
      tabsApi
        ...
        .then((response) => {
          ...
          console.info("[tabstride borrow] confirmation response", { tabId, allowed });
          settle(allowed);
        })
        .catch((err) => {
          if (settled) return;
          ...
          console.warn(
            "[tabstride borrow] sendMessage to candidate user window failed — trying next",
            {
              tabId,
