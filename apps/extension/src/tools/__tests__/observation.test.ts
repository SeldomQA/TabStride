...
import {
  type CdpAxNode,
  findInternalOverlayBackendNodeIds,
  handleGetHtml,
  handleScreenshot,
  handleSnapshot,
  parsePngDimensions,
  removeInternalAxNodes,
  renderAxTree,
  type ScreenshotDeps,
  shouldRender,
  stripDataUrlPrefix,
...
describe("overlay snapshot isolation", () => {
  it("collects backend node ids from the host and closed shadow descendants", async () => {
    const { cdp } = makeFakeCdp({
    ...
    await expect(findInternalOverlayBackendNodeIds(cdp, 4)).resolves.toEqual(
      new Set([900, 901, 902]),
    );
  });

  it("removes overlay AX nodes without removing business nodes", () => {
    const business: CdpAxNode = {
      nodeId: "business",
      backendDOMNodeId: 100,
      role: { type: "role", value: "button" },
      name: { type: "computedString", value: "Add Package" },
    };
    const overlay: CdpAxNode = {
      nodeId: "overlay",
      backendDOMNodeId: 901,
      role: { type: "role", value: "button" },
      name: { type: "computedString", value: "Stop agent" },
    };

    expect(removeInternalAxNodes([business, overlay], new Set([900, 901]))).toEqual([business]);
  });
});
...
describe("handleSnapshot", () => {
  function makeDeps(nodes: CdpAxNode[], overlayNode?: object) {
    const sendImpl = async (_tabId: number, method: string, _params?: object) => {
      ...
      if (method === "DOM.querySelector") return { nodeId: overlayNode ? 9 : 0 };
      if (method === "DOM.describeNode") return { node: overlayNode };
  ...
  it("excludes TabStride overlay controls from text and refs", async () => {
    ...
    const business: CdpAxNode = {
      nodeId: "1",
      role: { type: "role", value: "RootWebArea" },
      name: { type: "computedString", value: "Packages" },
      backendDOMNodeId: 100,
      childIds: ["2", "3"],
    };
    const addPackage: CdpAxNode = {
      nodeId: "2",
      parentId: "1",
      role: { type: "role", value: "button" },
      name: { type: "computedString", value: "Add Package" },
    ...
    const deps = makeDeps([business, addPackage, stopAgent], {
      nodeId: 9,
      backendNodeId: 900,
      shadowRoots: [
        {
    ...
    expect(res.text).toContain("Add Package");
    expect(res.text).not.toContain("Stop agent");
    expect(res.ref_count).toBe(2);
    expect(ctx.refStore.resolve("e1")).toBe(100);
    expect(ctx.refStore.resolve("e2")).toBe(200);
    expect(ctx.refStore.resolve("e3")).toBeNull();
  });
