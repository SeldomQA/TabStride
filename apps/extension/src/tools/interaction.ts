  ...
  try {
    ...
    if (automationBypassEnabled && deps.bypassOverlay) {
      try {
        ...
        console.debug("[tabstride interaction] overlay bypass disable failed", err);
      }
    }
  }
...
async function checkOverlayAtPoint(
  ...
  try {
    const hitTest = await cdp.send<{
      ...
      expression: `(function() {
        const overlayHost = document.querySelector("[data-tabstride-overlay]");
        return {
    ...
    console.debug("[tabstride interaction] overlay hit-test failed", err);
    return false;
  }
}
