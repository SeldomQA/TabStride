...
// returns a payload that mirrors the tabstride-protocol Rust structs.
...
function collectBackendNodeIds(node: CdpDomNode | undefined, into: Set<number>): void {
  if (!node) return;
  if (typeof node.backendNodeId === "number") {
    into.add(node.backendNodeId);
  }
  for (const child of node.children ?? []) collectBackendNodeIds(child, into);
  for (const shadowRoot of node.shadowRoots ?? []) collectBackendNodeIds(shadowRoot, into);
  for (const pseudo of node.pseudoElements ?? []) collectBackendNodeIds(pseudo, into);
  collectBackendNodeIds(node.contentDocument, into);
}

/**
 * Find every DOM backend node owned by TabStride's injected overlay.
 *
 * This is deliberately best-effort: page observation must still work when the
 * overlay is absent, Chrome refuses to expose a closed shadow root, or an older
 * browser does not support one of the DOM parameters.
 */
export async function findInternalOverlayBackendNodeIds(
  cdp: SharedCdpRunner,
  tabId: number,
): Promise<Set<number>> {
  const backendNodeIds = new Set<number>();
  try {
    const documentResult = await cdp.send<{ root?: { nodeId?: number } }>(
      tabId,
      "DOM.getDocument",
      { depth: 0, pierce: true },
    );
    const rootNodeId = documentResult.root?.nodeId;
    if (typeof rootNodeId !== "number") return backendNodeIds;

    const queryResult = await cdp.send<{ nodeId?: number }>(tabId, "DOM.querySelector", {
      nodeId: rootNodeId,
      selector: TABSTRIDE_OVERLAY_HOST_SELECTOR,
    });
    if (typeof queryResult.nodeId !== "number" || queryResult.nodeId === 0) {
      return backendNodeIds;
    }

    const description = await cdp.send<{ node?: CdpDomNode }>(tabId, "DOM.describeNode", {
      nodeId: queryResult.nodeId,
      depth: -1,
      pierce: true,
    });
    collectBackendNodeIds(description.node, backendNodeIds);
  } catch {
    // The overlay is an implementation detail, not a prerequisite for a
    // snapshot. aria-hidden remains the first line of AX isolation.
  }
  return backendNodeIds;
}

export function removeInternalAxNodes(
  nodes: CdpAxNode[],
  internalBackendNodeIds: ReadonlySet<number>,
): CdpAxNode[] {
  if (internalBackendNodeIds.size === 0) return nodes;
  return nodes.filter(
    (node) =>
      typeof node.backendDOMNodeId !== "number" ||
      !internalBackendNodeIds.has(node.backendDOMNodeId),
  );
}
 ...
 * `524288` value documented in the tabstride-protocol Rust struct so the
