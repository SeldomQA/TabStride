...
export async function scrollNodeIntoView(
  ...
  try {
    ...
    console.debug("[tabstride element-geometry] scrollIntoViewIfNeeded failed", err);
  }
...
async function visibleContentPolygon(
  ...
  try {
    ...
    console.debug("[tabstride element-geometry] getContentQuads failed", err);
  }
  try {
    ...
    console.debug("[tabstride element-geometry] getBoxModel failed", err);
  }
...
async function descendantBoundingRect(
  ...
  if (isRpcError(objectIdOrErr)) {
    console.debug(
      "[tabstride element-geometry] resolveNode for descendant bounds failed",
      objectIdOrErr,
    );
    return null;
  }
  try {
    ...
    console.debug("[tabstride element-geometry] descendant bounds failed", err);
    return null;
  }
}
