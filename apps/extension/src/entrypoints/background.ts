...
export default defineBackground(() => {
  ...
  if (typeof chrome.notifications?.onClicked?.addListener === "function") {
    ...
    console.warn(
      "[tabstride] chrome.notifications unavailable; borrow notifications will be skipped",
    );
  }
  ...
  if (typeof chrome.notifications?.onButtonClicked?.addListener === "function") {
    ...
    console.warn(
      "[tabstride] chrome.notifications.onButtonClicked unavailable; borrow Allow/Deny buttons will be inactive",
    );
  }
  ...
  void (async () => {
    ...
    console.error("[tabstride] controller failed to attach", err);
  });
  ...
  chrome.runtime.onConnect.addListener((connection) => {
    ...
    const post = (msg: PopupInbound) => {
      try {
        ...
        console.debug("[tabstride] popup post failed", err);
      }
    };
    ...
    connection.onMessage.addListener((raw: unknown) => {
      ...
      if (msg && typeof msg === "object" && "kind" in msg) {
        if (msg.kind === "set_port") {
          ...
          console.warn("[tabstride] set_port is not wired yet; ignoring", msg.value);
  ...
  if (import.meta.env.DEV) {
    const dbg = globalThis as unknown as {
      __tabstrideController?: ConnectionController;
      __bhSessions?: SessionManager;
      __bhDispatcher?: ToolDispatcher;
    };
    dbg.__tabstrideController = controller;
...
export async function handleOverlayInterrupt(
  ...
  try {
    ...
    console.warn("[tabstride] failed to send session.user_interrupt", err);
    return { ok: false };
  }
}
