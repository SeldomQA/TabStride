...
import {
  createIsolatedOverlayRoot,
  type IsolatedOverlayRoot,
} from "@/content/isolated-overlay-root";
...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    let isolatedRoot: IsolatedOverlayRoot | null = null;
    ...
    function mountOverlayUi() {
      ...
      reactRoot?.unmount();
      isolatedRoot?.remove();
      isolatedRoot = createIsolatedOverlayRoot(document, overlayCss);
      overlayHost = isolatedRoot.host;
      hostLossReported = false;
      reactRoot = ReactDOM.createRoot(isolatedRoot.container);
      renderOverlay();
    }
    ...
    function renderOverlay() {
      ...
      overlayHost?.toggleAttribute(
        TABSTRIDE_OVERLAY_CONTROL_BLOCKING_ATTRIBUTE,
        overlayState.controlVisible &&
          overlayState.activeHelp === null &&
          overlayState.automationBypassCount === 0,
      );
      reactRoot?.render(
        React.createElement(
          I18nextProvider,
    ...
    function handleInterrupt() {
      ...
      if (!sessionId) {
        console.warn("[tabstride overlay] interrupt requested with no active session id");
        return;
      }
      ...
      void sendInterrupt((msg) => chrome.runtime.sendMessage(msg), sessionId).then((reply) => {
        ...
        if (!reply.ok) {
          console.warn("[tabstride overlay] interrupt did not get a clean ack from daemon");
        }
      });
    }
...
async function anySessionLive(): Promise<boolean> {
  ...
  try {
    ...
    console.debug("[tabstride overlay] sessions-live flag read failed", err);
    return true;
  }
}
