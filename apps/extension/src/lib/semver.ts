...
//
// Wire format is `MAJOR.MINOR` (see `tabstride-protocol::system` in Rust), not full
 ...
 * `tabstride_protocol::system::compare_protocol`.
 */
 ...
 * (see `tabstride-protocol::system::parse_major` in
 * `crates/tabstride-protocol/src/system.rs`). Returns `null` when the leading
