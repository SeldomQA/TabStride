...
describe("ControlOverlay", () => {
  ...
  it("keeps operation logs out of the page overlay", () => {
    const { container } = render(
      <ControlOverlay
        visible={true}
        interrupting={false}
    ...
    expect(container.querySelector("[data-slot='control-overlay-log-toggle']")).toBeNull();
    expect(container.querySelector("[data-slot='control-overlay-stop-all']")).toBeTruthy();
  });
});
