...
describe("HelpRequestOverlay", () => {
  ...
  it("removes body content from collapsed width layout", () => {
    ...
    const body = container.querySelector(".tabstride-help-body") as HTMLElement;
    const styles = overlayStyles(container);

    expect(banner.getAttribute("data-collapsed")).toBe("true");
    expect(body.getAttribute("aria-hidden")).toBe("true");
    expect(styles).toContain('.tabstride-help-banner[data-collapsed="true"] .tabstride-help-body');
  ...
  it("keeps the same banner width when collapsed", () => {
    ...
    const banner = container.querySelector("[data-slot='help-request-banner']") as HTMLElement;
    expect(banner.getAttribute("data-collapsed")).toBe("true");
    expect(styles).toContain("--tabstride-help-banner-width: 420px");
    expect(styles).toContain("width: var(--tabstride-help-banner-width)");
  ...
  it("keeps collapsed header actions visible when the title is long", () => {
    ...
    expect(styles).toMatch(
      /\.tabstride-help-title\s*\{[^}]*min-width:\s*0;[^}]*overflow:\s*hidden;[^}]*text-overflow:\s*ellipsis;/s,
    );
    expect(styles).toMatch(/\.tabstride-help-header-actions\s*\{[^}]*flex-shrink:\s*0;/s);
  });
  ...
  it("renders a labelled drag handle affordance", () => {
    ...
    expect(handle.querySelector(".tabstride-help-drag-pill")).toBeTruthy();
  });
