...
tabstride extension and whatever channel it uses to talk to the local
daemon. v0.1 ships exactly one implementation —
[`ws-transport.ts`](./ws-transport.ts) — which speaks the JSON-line wire
format defined in `crates/tabstride-protocol/schema/*.json` over a plain WebSocket.
...
A `ProtocolFrame` is one of three JSON shapes (mirrors `tabstride-protocol::Frame`):
